class AceInput < SimpleForm::Inputs::Base
  ...
  def input
    ...
    input_html_options[:class] = [input_html_options[:class], 'hide'].join(' ')
    ...
    out << template.content_tag(:div, '', id: "ace_#{attribute_name}", class: "ace-input", style: base_styles, data: { target: target })
    out.html_safe
  end
end
